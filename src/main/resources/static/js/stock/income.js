$(document).ready(function() {
	...
	$("#incomeDel").on("click", () => {
		...
		if (confirm("정말로 삭제하시겠습니까?")) {
            ...
            var params = { state: "delete", inComes };
    ...
    $(".income-btn").on("click", function() {
        var bizNo = $(this).attr("data-bizNo");
        var _csrf = document.querySelector('input[name="_csrf"]').value;
        ...
        .done(function(data) {
            if (data.status) {		
                $("#bizName").text(data.result.bizName);
                $("#bizTel").text(data.result.bizTel);
    ...
    $("#cb_all").on("change", (e) => {
        if(e.target.checked) {
            $("tbody tr").each((i, v) => {
                ...
                $(v).find("td").eq(4).find("input").prop("readonly", false);
                $(v).find("td").eq(8).find("select").prop("disabled", false);
            });
        } else {
            $("tbody tr").each((i, v) => {
                $(v).find("th").find("input").attr("checked", false);
                $(v).find("td").eq(4).find("input").prop("readonly", true);
                $(v).find("td").eq(8).find("select").prop("disabled", true);
            });
        }
    });
    ...
    $(".cb").on("change", function(e) {
        if(e.target.checked) {
            ...
            $tr.find("td").eq(4).find("input").prop("readonly", false);
            $tr.find("td").eq(8).find("select").prop("disabled", false);
        } else {
